...
export default function EditCemeteryPage({
  ...
  const [formData, setFormData] = useState<CemeteryFormData | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const cemetery = cemeteries.find((c: any) => c.id === resolvedParams.id);
        ...
        if (cemetery) {
          ...
          // 유형 처리 (기존 type 문자열 또는 새로운 types 배열)
          const types = cemetery.types || 
            (cemetery.type ? cemetery.type.split(',').map((t: string) => t.trim()) : []);
          
          // 이미지 처리
          const mainImage = cemetery.mainImage || cemetery.image || '';
          const images = cemetery.images || [];
          
          setFormData({
            name: cemetery.name || '',
            types: types,
            address: address,
            hours: cemetery.hours || '09:00-18:00',
            mainImage: mainImage,
            images: images,
            description: cemetery.description || '',
            region: (cemetery.region as RegionKey) || '',
  ...
  const handleAdditionalImagesUpload = async () => {
    if (selectedAdditionalFiles.length === 0 || !formData) return;
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/admin/cemeteries/${id}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          locale,
          name: formData.name,
          type: formData.types.join(', '), // 기존 호환성
  ...
  if (!formData) {
    return <div className="text-center py-12">장지를 찾을 수 없습니다.</div>;
  }

  return (
    <div>
      <h1 className="text-3xl font-bold mb-6 text-gray-900">장지 수정</h1>
            ...
            <div className="space-y-3">
              ...
              {formData.mainImage && (
                <div className="mt-2">
                  ...
                  <p className="mt-2 text-xs text-gray-500">현재 대표 이미지: {formData.mainImage}</p>
                </div>
              )}
            </div>
          </div>
